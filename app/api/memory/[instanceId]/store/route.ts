...
import { storeMemoryEvent, getOrCreateMemoryConfig } from '@/lib/memory'
...
export async function POST(req: NextRequest, { params }: { params: { instanceId: string } }) {
  ...
  const { eventType = 'CONVERSATION', content, summary, channel, senderId, sessionId, importance, metadata } = body

  if (!content) return NextResponse.json({ error: 'content required' }, { status: 400 })

  const config = await getOrCreateMemoryConfig(instanceId)
  const id = await storeMemoryEvent(
    { instanceId, eventType, content, summary, channel, senderId, sessionId, importance, metadata },
    config.tier as 'STANDARD' | 'PRO'
  )
